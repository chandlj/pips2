...
# from datasets.kubricdataset import KubricDataset
from datasets.kubric.challenges.point_tracking.dataset import create_point_tracking_dataset
...
def test_on_fullseq(model, d, sw, iters=8, S_max=8, image_size=(384,512)):
    ...
    rgbs = (d['video'][0] + 1.) * (255. / 2.)
    trajs = d["target_points"][0]
    valids = (~d["occluded"][0]).astype(np.float32)
    ...
    rgbs = torch.from_numpy(rgbs).permute(0,3,1,2) # S,C,H,W
    ...
    trajs_g = trajs.cuda().float() # B,S,N,2
    valids = valids.cuda().float() # B,S,N
...
def main(
        ...
        dataset_location='/microtel/nfs/datasets/',
    ...
    dataset_x = tfds.as_numpy(res)
    iterloader_x = iter(dataset_x)
